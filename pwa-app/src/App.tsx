...
function Layout({ children }: { children: ReactNode }) {
  ...
  const [updater, setUpdater] = useState<null | ((reloadPage?: boolean) => Promise<void>)>(null);
  useEffect(() => {
  ...
  return <div className="app-shell">{!online && <div className="offline-banner">当前离线：可继续查看本地收藏、历史和详情</div>}{updater && <div className="update-banner"><span>发现新版本，是否立即更新？</span><div><button onClick={() => { void updater(true); }}>更新</button><button onClick={() => setUpdater(null)}>稍后</button></div></div>}<div className="content-shell">{children}</div><BottomNav/></div>;
}
